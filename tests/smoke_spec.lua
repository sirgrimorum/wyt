...
describe("WYTGoto knows where each file lives", function()
    ...
    it("opens the text and the parent of the section the cursor is in", function()
        local function norm(p) return (p:gsub("\\", "/")) end
        helpers.write(root .. "plan.wyt.md", helpers.read(root .. "plan.wyt.md") .. "\n## Group: One\n")
        helpers.write(root .. "one/config.wyt.yml", "lang: en\ntype: novel\ncontent_type: content\nsections: false\n")
        helpers.write(root .. "one/plan.wyt.md", "# One\n")
        vim.cmd("edit " .. vim.fn.fnameescape(root .. "one/plan.wyt.md"))
        assert.equals(norm(root .. "one/text.wyt.md"), norm((goto_target("text"))))
        assert.equals(norm(root .. "plan.wyt.md"), norm((goto_target("parent"))))
    end)
...
describe("the project files WYT opens", function()
    -- A modeline is executed on open, and these files hold pasted prose and
    -- whatever the model wrote back.
    it("never reads a modeline out of one", function()
        local root = helpers.project({})
        require("wyt.autocmd").setup()
        local ok, err = pcall(function()
            helpers.write(root .. "text.wyt.md", "Una linea.\n\n<!-- vim: set tabstop=7 : -->\n")
            vim.cmd("edit! " .. vim.fn.fnameescape(root .. "text.wyt.md"))
            assert.falsy(vim.bo.modeline)
            assert.has_no_match("^7$", tostring(vim.bo.tabstop))
        end)
        vim.api.nvim_create_augroup("WYT_Plugin", { clear = true })
        helpers.cleanup(root)
        assert(ok, err)
    end)
end)
